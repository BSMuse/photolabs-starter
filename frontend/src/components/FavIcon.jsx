...
const FavIcon = (props) => {
  // Destructure 'selected' and 'havePhotos' from props
  const { selected, havePhotos } = props;

  return (
    <svg width={havePhotos ? "20" : "24"}  height={havePhotos ? "18" : "21"}  viewBox="0 0 23 17" fill="none" xmlns="http://www.w3.org/2000/svg">
      {/* Conditional path element based on the 'selected' prop */}
      ...
      {
          havePhotos && // Conditional rendering of a circle if 'havePhotos' is true
          <circle cx="21" cy="4" r="2.75" fill="greenyellow" stroke="#C80000" strokeWidth="0.5"/>
      }
    </svg>
  );
};
